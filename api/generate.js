...
module.exports = async function handler(req, res) {
  ...
  try {
    const { messages, max_tokens, access_code } = req.body;
    const code = (access_code || '').trim().toUpperCase();

    if (!code) return res.status(401).json({ error: 'Kode akses diperlukan.' });

    if (PERMANENT_CODES.includes(code)) {
      // valid, lanjut
    } else if (SINGLE_USE_CODES.includes(code)) {
      const db = getClient();
      const used = await db.get('used:' + code);
      if (used) return res.status(401).json({ error: 'Kode akses sudah digunakan.' });
      // Tidak mark used di sini — sudah dimark saat validate
    } else {
